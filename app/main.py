...
def run_pipeline():
    ...
    run_cmd('git config user.name "security-agent-bot"'.split())
    run_cmd('git config user.email "security-agent-bot@users.noreply.github.com"'.split())

    run_cmd(f"git add {target_file}".split())
    run_cmd('git commit -m "fix(security): AI-generated remediation"'.split())
    
    token = os.getenv("GH_TOKEN")
    repo = os.getenv("GITHUB_REPOSITORY")

    run_cmd(f"git remote set-url origin https://x-access-token:{token}@github.com/{repo}.git".split())
    run_cmd(f"git push origin {branch_name}".split())
